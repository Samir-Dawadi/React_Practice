export const RowSkeleton = ({ rows = 7, cols, showAction = true }: Readonly<{
    rows?: number,
    cols: number,
    showAction?: boolean
}>) => {
    ...
    return [...Array(rows)].map((_, i: number) => (
        <tr key={i}>
            ...
            {
                showAction ? <td className="p-2 border-r border-gray-500 border">
