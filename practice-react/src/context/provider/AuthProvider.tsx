...
//         try {
//             const loggedInUser = await axiosInstance.get("auth/me") as {
//                 data: IUserDetail
//             }
//             setAuthUser(loggedInUser.data)
...
//                 ...credentials,
//                 expiresIn: 24 * 60
//             }) as { accessToken: string, message: string, meta: null }
    ...
    const login = async (credentials: ICredentials): Promise<IUserDetail | void> => {
        try {
            const response = await axiosInstance.post("auth/login", {
                ...credentials,
                expiresInMins: 24 * 60,
            }) as { data: { accessToken: string }, message: string, meta: null }
            console.log("Login Response:", response);
            console.log("Access Token:", response.data.accessToken);
            Cookies.set("Auth_key_61", response.data.accessToken, {
                expires: 1,
                sameSite: "lax",
                secure: true,
            });
